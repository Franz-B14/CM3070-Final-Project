...
* Loss of communications is not interpreted as evidence that a hazard has cleared.
* The responder's received gateway sequence is currently RAM-only; replay protection therefore restarts with the responder. This remains a known prototype limitation.
...
* no operational emergency-service integration;
* no redundant gateway;
* no responder acknowledgement/telemetry channel;
* prototype rather than surveyed responder/gateway siting;
* simplified hazard thresholds requiring field calibration;
* Trust/ML work still in progress;
* replay state on the responder is not persisted;
* CAP messages are `Exercise` messages only.

The project should therefore not be deployed as a safety-critical warning system without further engineering, validation and regulatory review.
